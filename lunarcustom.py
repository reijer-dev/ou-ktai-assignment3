...
class LunarLanderCustom(gym.Env, EzPickle):
    """
    ## Description
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def reset(
        ...
        *,
        ...
        valid_shift = max(min(SHIFT, CHUNKS // 2 - 1), -(CHUNKS // 2 - 1))
        self.helipad_x1 = chunk_x[CHUNKS // 2 - 1 + valid_shift]
        self.helipad_x2 = chunk_x[CHUNKS // 2 + 1 + valid_shift]
        self.helipad_y = H / 4
        height[CHUNKS // 2 - 2 + SHIFT] = self.helipad_y
        height[CHUNKS // 2 - 1 + SHIFT] = self.helipad_y
        height[CHUNKS // 2 + 0 + SHIFT] = self.helipad_y
        height[CHUNKS // 2 + 1 + SHIFT] = self.helipad_y
        height[CHUNKS // 2 + 2 + SHIFT] = self.helipad_y
        smooth_y = [
